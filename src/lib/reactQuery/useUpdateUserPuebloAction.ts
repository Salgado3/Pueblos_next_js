...
const addPuebloAction = async (userId: string, puebloId: string, actionType: string) => {
  const supabase = createClient()
  const { error } = await supabase
    .from('user_pueblo_actions')
    .insert([{ user_id: userId, pueblo_id: puebloId, action_type: actionType }]);
  if (error) throw new Error(error.message);
};

const removePuebloAction = async (userId: string, puebloId: string, actionType: string) => {
    const supabase = createClient()
...
const useUpdateUserPuebloAction = (userId: string, puebloId: string) => {
  ...
  return useMutation({
    mutationFn: ({ isActioned, actionType }: { isActioned: boolean, actionType: string }) => {
